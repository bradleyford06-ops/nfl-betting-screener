...
- **Spreads:** real, monotonic edge — bigger disagreements win more often. Live threshold flags disagreements of 8+ points (157 bets in the backtest, 57.3% win rate, +11.4% ROI — updated 2026-08-31 after the relocation-bug fix below, up slightly from the original 156/56.4%/+9.6%).
- **Moneylines:** the original spread-based approach is disabled — see the "Moneyline root-cause investigation" and "New moneyline model" notes further below for what replaced it.
- **Totals:** now has a real, if modest, edge — see "Totals deep dive" immediately below. Was flat/unproven (~49-50% win rate) before that fix.
...
**A real bug found and fixed while investigating totals (2026-08-31):** `model/power_ratings.py` had no handling for three franchise relocations in the data window (Raiders OAK→LV 2020, Chargers SD→LAC 2017, Rams STL→LA 2016) — right after each move, the "new" team started from zero rating history instead of carrying its franchise's prior games forward. This produced occasional wildly unrealistic predictions on thin, noisy samples: LV had only 6 games of history by week 8 of 2020 (should have been 17), producing a 68.1-point predicted total for LV @ CLE (actual: 22) — this is what Bradley noticed and flagged. Fixed with a `RELOCATION_MAP`/`canonical_team` (the same fix `model/nfl_elo_ratings.py` already had from day one — now both models share one definition, in `power_ratings.py`). Re-backtesting confirmed the fix is a clean win: spread's real edge *improved* slightly (56.4%→57.3% win rate, +9.6%→+11.4% ROI), the count of unrealistic (58+ point) predicted totals dropped from 130 to 116 across 2010-2024, and the single worst offender (68.1) is gone. Total's own already-flat numbers moved slightly (within noise) — this bug was never the main reason totals lacked an edge; see below for what was.

**Totals deep dive (2026-08-31) — found a real, if modest, fix.** Bradley asked for a deep dive into why totals shows no edge. Tested the obvious hypothesis first — that the model is missing weather/dome effects — and it looked promising at first (domes score more than predicted, high wind scores less), but **failed honest out-of-sample validation**: fitting the correction on 2010-2018 and testing fresh on 2019-2024 made almost all the improvement disappear, and the fitted coefficients themselves were unstable between windows (one stadium-type adjustment went from -1.81 to -0.05 depending which years it was fit on) — ruled out as overfitting, not a real effect.

**What did hold up:** only trust a total pick when the model ALSO disagrees strongly with the market's spread in that same game — a total edge on its own is mostly noise, but paired with a genuine spread disagreement (the same 8-point bar already proven for spread) it's real signal. Validated properly: tested a looser version first (total edge≥6 + spread edge≥6) which looked good in a two-era check, but that finding **partly evaporated once the relocation bug above was fixed** — some of its "hits" turned out to be the same noisy relocation-era predictions, a useful lesson in how one bug can manufacture a fake pattern in an unrelated finding. Re-validated on the bug-fixed model at a stricter 8-point bar on both sides: over the full 2010-2024 window (192 bets), 56.2% win rate, +9.4% ROI, and — the important check — **positive in both independent halves** (61.2%/+19.4% in 2010-2017, 52.3%/+1.5% in 2018-2024), unlike the looser rule which flipped sign between halves. On the standard 2019-2024/2017-2018-burn-in protocol used for spread's headline number, the same rule gives a thinner but still positive 87 bets, 51.7% win rate, +0.3% ROI.

**Shipped live (2026-08-31):** `TOTAL_EDGE_THRESHOLD` raised to 8.0 (from 4.5), plus a new `TOTAL_SPREAD_CONVICTION_THRESHOLD = 8.0` co-filter (`model/power_ratings.py`'s `total_conviction_ok`, wired into `screener/pipeline.py`). This meaningfully shrinks how many total picks get flagged (~13/season at this bar vs. the old flat rule's much larger volume) in exchange for those picks being backed by real, two-era-validated evidence instead of "kept live despite no edge." Treat as a genuine but still modest edge — thinner and weaker in the recent era than spread's, not yet in the same tier of confidence.
...
- **NFL:** spread has real backtested edge (improved slightly by the 2026-08-31 relocation-bug fix). Moneyline: the original spread-based model is disabled (root-caused, not just untested — see the "Moneyline root-cause investigation" note above); replaced with a new Elo-based model, live but labeled speculative (modest accuracy improvement, still behind the market — see "New moneyline model" note above). Total: now has a real, if modest, edge behind a spread-conviction co-filter (see "Totals deep dive" above) — fewer picks than before, but backed by two-era-validated evidence instead of "no edge, kept anyway." Props: trend model (all stats) + coverage model (receptions/receiving yards only), both backtested on historical data but not yet validated against real live prop lines.
- **CFB:** spread has real backtested edge (~54%, +3.3% ROI). Total is split by side (2026-08-31, see `model/cfb_power_ratings.py`): Under is also a real, two-era-validated edge (~55-56% win rate, +5-7% ROI) and lands in the main results; Over stays speculative (no edge, gets worse with bigger disagreements) and is kept visible per Bradley's explicit choice. A separate real reliability bug (a transient API outage silently corrupting the ratings cache) was found and fixed during this investigation — see the same file for details. Own opponent-adjusted power rating, validated against SP+.
...
**Reliability infrastructure (added 2026-08-24 after the CFB API-key bug ran silently for two days):** a caught, per-sport failure (screening or reconciliation) now sends a distinct `send_partial_failure_alert` email instead of degrading silently — see `email_report/error_alert.py`. Both GitHub Actions workflows now `git pull --rebase` before pushing, since NFL/CFB/MLB (fixed 9am) and NHL (dynamic time) push to the same repo independently and could otherwise collide. The dashboard is built from the permanent ledger's currently-open picks (not a single run's in-memory results) specifically so multiple independently-scheduled runs can't clobber each other's sections — see `dashboard/build_data.py`. `screener/fetch_cfb_stats.py` also now refuses to cache a fetch where an already-completed season's year failed (`_reject_stale_year_failures`, added 2026-08-31) — a transient cfbd outage used to silently poison the schedule cache with one frozen, years-stale season; now it's a hard, visible failure that the existing alert machinery catches instead.
**Known issue to watch (2026-09-02):** the daily `screener.yml` workflow has three backup cron times (13:00/15:00/17:00 UTC) specifically because GitHub occasionally skips a scheduled trigger silently. On 2026-09-02, both the 13:00 and 15:00 triggers were skipped in the same day (worse than the single-miss pattern that originally justified three backups) — resolved by manually triggering (`gh workflow run screener.yml`), but if this recurs, consider a fourth backup time or an explicit "no successful run by noon ET" alert.
...
**Next step:** two candidates flagged for the same deep-dive treatment that fixed NFL/CFB totals this session — NHL puck line (explicitly known to be a structural artifact, not real edge) and MLB moneyline/total (speculative, no edge found) — neither started yet. Also worth a quick check that the temporary CFB Under-pick duplicate (see `model/cfb_power_ratings.py`'s Over/Under split note) has cleared from the dashboard on its own. Longer-term: NHL needs its shots-on-goal props and reconciliation once its season starts; NFL/CFB props still need validation against real live lines once enough of the season has real data; NFL prop categories that were never backtested (TDs, completions, attempts) still sit on a flat, un-calibrated guess and could get the same synthetic-backtest calibration treatment already used for receiving/rushing/passing yards.
