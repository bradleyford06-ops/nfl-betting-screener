...
**Status:** Fully automated and live, confirmed working as of 2026-08-18. Runs daily via GitHub Actions (repo: bradleyford06-ops/nfl-betting-screener), emails standouts to bradleyford5@hotmail.com, and publishes an interactive dashboard to GitHub Pages (https://bradleyford06-ops.github.io/nfl-betting-screener/) — every week browsable, drill into any game for its props, season performance tracked via a permanent ledger that auto-reconciles against real results. The first real scheduled run (2026-08-17) actually crashed — an unprotected team-name lookup with no fallback took down the whole run when its data source hiccupped (the error-alert safety net did fire correctly, so Bradley wasn't left in the dark). Fixed with a static 32-team fallback, then re-verified with another live triggered run: email sent, dashboard + ledger committed back successfully. Don't assume "built" means "working" for this pipeline going forward — it already broke once in production; keep verifying with real runs, not just local tests, when touching anything in the daily path. Spread screening has real backtested evidence of an edge; moneyline is disabled; totals stay on by product decision despite no proven edge. Player props run two independent signals (trend + coverage) for receptions/receiving yards, trend-only for other stats — both verified against real historical data, but neither tested against real live prop lines yet, since sportsbooks don't post NFL prop lines this far before the season.
**Outstanding, not on us:** Bradley needs to upgrade the Odds API plan himself (payment details required) — the free 500-credit/month tier is too small for daily runs plus per-game player-prop calls. Not yet confirmed done as of 2026-08-18; check this early in a future session, since if it's still on the free tier the daily Action may start failing or returning incomplete odds once real props start posting and volume increases.
**Next step:** Check whether Bradley upgraded the Odds API plan. Then, once the season is closer and player props start appearing, verify both props screeners end-to-end against real lines — this is the single biggest remaining unknown in the whole pipeline. Consider re-backtesting periodically as more of the 2026 season accumulates. Also worth backtesting the coverage model's remaining un-tested combos (e.g. TD markets) if there's appetite to extend it further.
**Phase:** 2 of 2 — both phases done and verified live. Phase 1 (working, backtested screener) and Phase 2 (automated daily delivery + dashboard) are both confirmed running in production. Remaining work is validation against real season data as it accumulates, not new infrastructure.
