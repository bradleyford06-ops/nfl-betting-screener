...
**New moneyline model — scoped, not yet built (as of 2026-08-30).** Bradley's explicit call after the investigation above: don't try to patch the existing spread-based model for moneyline — build a separate model designed for win/loss prediction from the ground up. Leading candidate approach: an Elo-style rating (updates from actual win/loss outcomes with margin-of-victory dampening, not raw scoring differential) with a QB-value adjustment layer, the same general approach FiveThirtyEight's well-documented NFL Elo used — directly targets the exact gap the investigation found (backup-QB/situational swings that a scoring-margin model can't see, and a growing mid-season gap vs. the market). Needs its own scoping pass and backtest before going live, same process as every other model in this project.
...
- **NFL:** spread has real backtested edge; moneyline disabled — root-caused, not just untested (2026-08-30 investigation found it's a structural gap, not a fixable bug; see the "Moneyline root-cause investigation" note above). A from-scratch, win/loss-focused replacement model is scoped but not yet built. Total live but speculative (no edge). Props: trend model (all stats) + coverage model (receptions/receiving yards only), both backtested on historical data but not yet validated against real live prop lines.
