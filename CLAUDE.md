...
- **CFB:** spread has real backtested edge (~54%, +3.3% ROI); total live but speculative (no edge — same underlying conclusion as before, but see the 2026-08-31 "Totals deep dive" note in `model/cfb_power_ratings.py` for a real reliability bug found and fixed while investigating why predicted totals showed such wide swings). Own opponent-adjusted power rating, validated against SP+.
