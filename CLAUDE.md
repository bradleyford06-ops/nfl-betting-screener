...
**Status:** Fully automated and live, confirmed working as of 2026-08-21. Runs daily via GitHub Actions (repo: bradleyford06-ops/nfl-betting-screener), emails standouts to bradleyford5@hotmail.com, and publishes an interactive dashboard to GitHub Pages (https://bradleyford06-ops.github.io/nfl-betting-screener/) — every week browsable, drill into any game for its props, season performance tracked via a permanent ledger that auto-reconciles against real results and is already broken down by strategy (spread/total/props trend/speculative/coverage) on the dashboard. Two real production bugs have been found and fixed so far: an unprotected team-name lookup crashed the first scheduled run (2026-08-17, fixed with a static fallback), and the screener was flagging edges across the *entire season* instead of just the upcoming week, since the odds API returns the whole season in one call and nothing filtered by week (2026-08-21, fixed with `get_current_week()` in `screener/pipeline.py`, also cut API usage substantially as a side effect). Both fixes were verified against real triggered GitHub Actions runs, not just local tests. Don't assume "built" means "working" for this pipeline — keep verifying with real runs when touching anything in the daily path. Spread screening has real backtested evidence of an edge; moneyline is disabled; totals stay on by product decision despite no proven edge. Player props run two independent signals (trend + coverage) for receptions/receiving yards, trend-only for other stats — both verified against real historical data, but neither tested against real live prop lines yet, since sportsbooks don't post NFL prop lines this far before the season.
**Outstanding, not on us:** Bradley needs to upgrade the Odds API plan himself (payment details required) — the free 500-credit/month tier is too small once real player-prop volume shows up. Deliberately deferred (2026-08-17 session): Bradley chose to upgrade right before the season starts rather than now, to save money — not an oversight, don't re-flag this as urgent.
**Scope decisions (2026-08-21):** reviewed extending to more prop markets (anytime TD, kicker props, defensive props, INTs, combo yardage) and a DFS lineup-optimizer feature (confirmed technically feasible — projections exist, a free optimizer library exists, salary data would need a manual weekly CSV export). Bradley decided not to pursue either for now — don't re-propose without him raising it again.
**Next step:** Bradley wants to build a new model/strategy next, to be added to this same project. Not yet scoped — ask what it is, then follow the same process used for every prior model here (understand the reasoning, check real data feasibility, build, backtest, calibrate, integrate as an independent addition). Separately, once the season is closer and player props start appearing, verify both props screeners end-to-end against real lines — still the single biggest remaining unknown in the whole pipeline.
**Phase:** 2 of 2 — both phases done and verified live. Remaining work is the next model Bradley wants to add, plus validation against real season data as it accumulates.
